			...
			fs.readFile(folderHTML, 'utf-8', function (error, data) {
				...
				jsdom.env( data,
				  ...
				  function(errors, window) {
					 ...
					 res.render('edit', {username: req.user.name, slidesText: slidesText, title: slideshow.title});
				  }
				);
	...
	slideshowDB.findById(req.query.id, function(err, slideshow) {
		if (err) {
			...
			fs.readFile(folderHTML, 'utf-8', function (error, data) {
				...
				res.render('edithtml', {username: req.user.name, html:data, id:req.query.id, title: slideshow.title, alert: false, type:null});
			});
		}
	});
}
		...
		if (err) {
			...
			fs.writeFile(folderHTML, req.body.editorvalue, function(err) {
				if (err) {
					...
					res.render('edithtml', {username: req.user.name, html: req.body.editorvalue, id:req.query.id, title: slideshow.title, alert: "Your data has been successfully saved.", type:"succes"});
				}
			});
		}
	});
...
exports.editstyle=function(req,res) {
	...
	slideshowDB.findById(req.query.id, function(err, slideshow) {
		if (err) {
			...
			fs.readFile(folderHTML, 'utf-8', function (error, data) {
				// console.log(req.query.id + " "+ data);
				res.render('editstyle', {username: req.user.name, html:data, id:req.query.id, title: slideshow.title, alert: false, type:null});
			});
		}
	});
}
	...
	slideshowDB.findById(req.query.id, function(err, slideshow) {
		if (err) {
			...
			fs.writeFile(folderHTML, req.body.editorvalue, function(err) {
				...
				res.render('editstyle', {username: req.user.name, html: req.body.editorvalue, id: req.query.id, title: slideshow.title, alert: "Your data has been successfully saved.", type:"succes"});
											...
											optionDB.findById(question.answeroptions[j],function(err,option) {
												...
												if (newquestionob.answeroptions.length==question.answeroptions.length) {
													...
													if (questions.length==slideshow.questions.length) {
														console.log("########################2");
														res.render('editQuestions', {arrayquestions: questions, username: req.user.name, title:slideshow.title});
													}
												}
											});
										}
