...
module.exports = {
  ...
  savePluginSessionData: coroutine(function *savePluginSessionDataGen(pluginName, sessionId, payload) {
    var pluginInfo = yield db.model('PluginInfo').findOne({
    ...
    if (pluginInfo) {
      pluginInfo.data = payload;
      pluginInfo.save(function(err) {
      ...
      yield db.model('PluginInfo').create(pluginInfo);
    }
  }),
  ...
  getPluginSessionData: coroutine(function *getPluginSessionDataGen(pluginName, sessionId) {
      ...
      return (pluginInfo)? pluginInfo.data : null;
  }),

  // function to delete all pluginInfo data, just for testing
  deletePluginSessionData: function() {
    db.model('PluginData').remove({}, function(err) {
      if (err) console.log(err);
    });
  },
