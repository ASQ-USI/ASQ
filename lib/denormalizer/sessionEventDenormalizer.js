...
var SessionEventCockpit = db.model('SessionEventCockpit');
var SessionEventBeamer = db.model('SessionEventBeamer');
var renameIdAndRemoveVersion = require('../utils/format.js').renameIdAndRemoveVersion


function getUserFromWhitelistEntry(userid, field, session){
  field = field || '_id';
  var query = {};
  query[field] = userid;
  if(session){
  ...
  return  WhitelistEntry
    ...
    .then(function getUserFromWhitelistEntryRes(wlistEntry){
      ...
      user.whitelistEntry = wlistEntry._id;
      return user;
    })
}

var denormalizeGotoSessionEvent = coroutine(function *denormalizeGotoSessionEventGen (sEvent){
  var user = yield getUserFromWhitelistEntry(sEvent.data.user, 'user', sEvent.session);

  sEvent.origSessionEvent = sEvent._id;
  sEvent.user = user;
  sEvent.slide = sEvent.data.slide;
  sEvent.fragment = sEvent.data.fragment;
  delete sEvent.data;
  delete sEvent._id;

  return SessionEventCockpit.create(sEvent);
});

var denormalizeExerciseSessionEvent = coroutine(function *denormalizeExerciseSessionEventGen (sEvent){
  var userPromise = getUserFromWhitelistEntry(sEvent.data.user, 'user', sEvent.session);
...
var denormalizeAnswerSessionEvent = coroutine(function *denormalizeAnswerSessionEventGen (sEvent){
  ...
  return Promise.all([
      SessionEventCockpit.create(sEvent),
      SessionEventBeamer.create(sEvent)
  ]);
});
...
var denormalizeLiveAppSessionEvent = coroutine(function * denormalizeLiveAppSessionEvent(sEvent){
  // console.log("ENTERED IN sessionEventDenormalizer");
  // console.log("-----   sEvent   -----");
  // console.log(sEvent);
  sEvent.origSessionEvent = sEvent._id;
  if (sEvent.type === "student-perception-change")
    sEvent.value = sEvent.data.value;
  sEvent.user = sEvent.data.user;
  delete sEvent.data;
  delete sEvent._id;
  // console.log("-----   sEvent Changed  -----");
  // console.log(sEvent);
  return SessionEventCockpit.create(sEvent);
});
module.exports = {
  denormalizeEvent : function (sEvent){
    ...
    switch(sEvent.type){
      ...
      case "student-perception-change":
        denormalizeLiveAppSessionEvent(sEvent);
        break;
