...
var authorizeLive = {
	'public' : function publicLiveAuthroize(handshakeData, callback) {
	...
	'anonymous' : function anonymousLiveAuthroize(handshakeData, callback) {
		// check if cookie was sent.
    if (!handshakeData.headers || !handshakeData.headers.cookie) {
...
function liveAuthorize(handshakeData, callback) {
  ...
  var Session = db.model('Session', schemas.sessionSchema);
  Session.findById(handshakeData.query.sid, '_id activeSlide authLevel',
  function onSession(err, session) {
  	if (err) { callback(err, false); }
  	handshakeData.session = session;
