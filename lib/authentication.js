    ...
    if (req.isAuthenticated()){
      ...
      GuestUser.findOne({ token : token }, '_id screenName', function (err, user) {
        ...
        if (!user) {
          ...
          appLogger.info('Old user connected')
          deferred.resolve(user);
        }
      });
    }
    deferred.promise.then(
    function generateWhiteListEntry(user) {
      appLogger.debug("{uid : " + user._id + ", session : " + req.liveSession._id + "}")
      appLogger.debug("{token : " + token + ", screenName : " + user.screenName + "}")
      return WhitelistEntry.findOneAndUpdate({
        uid : user._id,
  ...
  WhitelistEntry.findOne({
    ...
    role    : 'presenter'
      ...
      Session.findById(handshakeData.query.sid, function(err, session) {
        ...
        handshakeData.token      = entry.token;
