...
exports.getPresentationStats = gen.lift (
  ...
  if(! (presentation instanceof Slideshow))
  {
    ...
    try{
      ...
      appLogger.error("Presentation %s not found", pId);
...
exports.getSessionStats = gen.lift (
  ...
  if(! (session instanceof Session))
  {
    ...
    try{
      ...
      appLogger.error("Session %s not found", sId);
      appLogger.error(err.message, { err: err.stack });
    }
  }
  ...
  if(! (session.slides instanceof Slideshow)){
     try{
     yield nodefn.lift(session.populate.bind(session))('slides');
    }catch(err){
      appLogger.error("Error populating slides for session &s", session._id);
      appLogger.error(err.message, { err: err.stack });
    }
  }
  ...
  var assessments = yield Assessment.find({
   ...
   console.log("Da length", userData.length)
  ...
  assessments.forEach(function(assessment){
    ...
    for(var i = 0, l = userQuestions.length; i<l; i++){
      if(userQuestions[i].id === assessment.question.toString()){
        ...
        userQuestions[i].answered = (assessment.score === 100)
          ? 'correct'
          : 'wrong';
        break;
      }
  ...
  return {
    users: userData,
    questions : questions,
    liveId : ((session.endData ==null) ? session.id : null),
    liveData: ((session.endData ==null) ? true : false)
  }
});
