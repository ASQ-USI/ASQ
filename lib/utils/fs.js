...
function getFirstHtmlFile(searchPath){

  var deferred = when.defer();
  var hasHTML = false;
  pfs.readdir(searchPath).then(
    function(list){
      hasHTML = list.sort().some(function (file){
        var stat =fs.statSync(searchPath + '/' + file);
        if (stat && stat.isFile() && getExtension(file)=="html"){
          deferred.resolve(path.join(searchPath,  file));
          return true;
        }
      });
      if(! hasHTML) {
...
// This function also exists in logger.js, the reason we duplicate
