...
var submit2 = function(io, socket, ) {
  logger.debug('Submission');
  when.all([
    getSession(socket),
    getToken(socket),
    Exercise.findById(evt.exercise.id).exec()
  ]).then(
    function onData(data) {
      return submission.process(data[0], data[1], evt.exercise);
  }).then(
    function something() {
      //...
    })
...
//     );
// };
ObjectId("5347e311897f7edf07ac8b8e")
"vK5sLqFsDpPntGGtW8vsYp2v"
