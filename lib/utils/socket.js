...
var assessed = function(io, socket, evt) {
  ...
  when.all([
    ...
    function onData(data) {
      return assessment.save(data[0], data[1], evt.assessment);
  }).then(
    function nextAssessment(data) {
      var session  = data[0];
      var exercise = data[1];
      var token    = data[2];
      var progress = data[3];
      ...
      return assessment.render(session, exercise, token);
  }).then(
...
var foloDisconnect = function(io, socket) {
  ...
  function updateProgresses(data) {
    return submission.progress.updateDisconnect(data[0], data[1]);
  }).then(
