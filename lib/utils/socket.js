  ...
  when.all([
    ...
    function handleAssessment(data) {
      ...
      if (self || peer) { // Add answers for peers
        ...
        return assessment.job.enqueue(progress.session, exercise,
          Object.keys(answers).map(function(i) { return answers[i]; }))
          .then(function onAddedJob(jobs) {
            console.log('added jobs');
            return assessment.job
              .getNextAssessmentJob(progress.session, exercise, token);
          });
      } else {
        return when.resolve(null);
      }
...
var assessed = function(io, socket, evt) {
  ...
  var session  = null;
  var assessor = null;
  var exercise = evt.assessment.exercise;
  ...
  when.all([
    ...
    function termninateJobs(assessments) {
      return assessment.job
        .terminateJobsAndUpdateProgress(session, exercise, assessor, assessee);
  }).then(
    function nextAssessment(progress) {
      ...
      console.log('getting job for', progress.session, exercise, assessor)
      return assessment.job.getNextAssessmentJob(progress.session, {_id: exercise}, assessor);
  }).then(
    ...
    function sendAssemnet(html) {
      console.log('got html');
      if (html) {
        console.log('really got html')
        socket.emit('asq:assess', { html: html, exercise: evt.exercise.id });
      }
  }).then(null,
    function onError(err) {
...
var ctrlDisconnect = function(io, socket) {
  ...
  function delAndNotify(data) {
    return deleteAndNotify(io, socket, data, 'ctrl', 'ctrl', 'folo', 'wtap');
  },
...
var foloDisconnect = function(io, socket) {
  when.all([
    ...
    function delAndNotify(data) {
    return deleteAndNotify(io, socket, data, 'folo', 'ctrl', 'folo', 'wtap');
  }).then(
...
var wtapDisconnect = function(io, socket) {
  ...
  function handleDisconnect(sessionId) {
    return deleteAndNotify(io, socket, [sessionId, 'Wtap client'], 'wtap', 'ctrl');
  },
