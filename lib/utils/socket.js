  ...
  when.all([
    ...
    .then(
      function updateAnswer(info) {
        ...
        function selfAssess(answer){
          var name = getDisplayName(socket);
...
var foloAssess = function(io, socket, evt) {
  getSessionId(io, socket).then(
    function updateAssessment(sessionId) {
      return Assessment.findOneAndUpdate(
        { _id: evt.id, session: sessionId },
        { score : evt.score }
      ).exec();
    }).then(function nextAssessment(assessment) {
