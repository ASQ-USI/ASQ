...
var submit = function(io, socket, evt) {
  ...
  when.all([
    ...
    }).then( // Generate self assessment
      function getUserId(answer){
        //TODO: Change from DisplayName to token.
        var whiteListQuery = WhiteListEntry.findOne({
          displayName: data[1]
        }).exec();
        return when.join([whiteListQuery, answer]);
    }).then(
      function selfAssess(assesData) {
         var selfA = new Assessment({
