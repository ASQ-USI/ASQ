...
var getTokenAndScreenName = function(socket, callback) {
  ...
  getToken(socket).then(
    function onToken(token) {
      ...
      return WhitelistEntry.findOne({ token : token }, 'screenName').exec();
    })
  .then(
...
var goto = function(io, socket, evt) {
  ...
  getSession(io, socket)
    ...
    .then(function(results) {
      ...
      if (currentSession.activeStatsQuestions) {
        Question.find({_id: {$in: currentSession.activeStatsQuestions}}).exec()
        .then(
          function onQuestions(questions) {
            return when.all(questions.map(function onQuestion(question) {
              return question.getStats(currentSession._id);
            }));
        }).then(
