  ...
  getSession(io, socket)
    ...
    .then(function(results) {
      ...
      if (!! currentSession.activeStatsQuestions.length) {
       return Question.find({_id: {$in: currentSession.activeStatsQuestions}}).exec()
      }
    ...
    .then(
      function onQuestions(questions) {
        ...
        for (var i = questions.length; i--;) {
          o[questions[i]._id] = questions[i].getStats(currentSession._id);
        }
        return wkeys.all(o);
    }).then(
      function onStats(stats) {
        if (!! stats) {
          console.log('stats')
          console.log(require('util').inspect(stats, { showHidden: true, depth: null }));
          evt.stats = stats;
        }
        console.log('event');
        console.dir(evt);
