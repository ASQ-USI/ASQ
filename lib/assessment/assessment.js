...
*/
var terminateJobsAndUpdateProgress = gen.lift(function *terminateJobsAndUpdateProgressGen
  ...
  var savedJobs = yield  when.map(jobs, function saveJob(job) {
    job.status = 'finished';
...
var activateJob = function(job) {
  if (! job) {
    process.nextTick(function onNoJob() { return when.resolve(null); });
    return;
  }
  job.status = 'active';
