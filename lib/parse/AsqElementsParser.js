...
Parser.prototype.asqify = function(html, presentationFramework){
  ...
  this.injectLiveApp($,presentationFramework);
  this.injectRoleInfo($);
...
Parser.prototype.injectLiveApp = function($, framework){

  let isLocalBowerComponents = true;
  const asqElementImports = $('link[rel="import"]').filter((idx, el) => {
    return $(el).attr('href').endsWith('asq-elements.html')
  })

  // if asq-elements.html are local to the presentation use a local live-app
  if(asqElementImports.length >= 1){
    const href =  asqElementImports.eq(0).attr('href');
    isLocalBowerComponents =  /^(.\/)?bower_components/.test(href); 
  }

  if(isLocalBowerComponents){
    $('head').append('<link rel="import" href="bower_components/asq-live-app/asq-live-app.html">')
  }else{
    $('head').append('<link rel="import" href="/asq-live-app/asq-live-app.html">')
  }

  //if it's impress
  if(framework == 'impress.js'){
    $('#impress').after('<asq-live-app></asq-live-app>')
  }else if(framework == 'reveal.js'){
    $('#reveal').after('<asq-live-app></asq-live-app>')
  }else{
    throw new Error('missing or unknown framework');
  }
