...
Parser.prototype.createConfiguration = function(option){
  return hooks.doHook('create_configuration', option).then(function(option){
    return Promise.resolve(option);
  });
};

Parser.prototype.createSlidesConfiguration = function(option){
  console.log('createSlidesConfiguration');
  return hooks.doHook('create_slideshow_conf', option).then(function(option){
    return Promise.resolve(true);
  });
};

Parser.prototype.getPresentationConf = function(html){
  console.log(' -> getPresentationConf');
  var $ = cheerio.load(html, {decodeEntities: false});
  var el = $('asq-configuration');
  return $(el).attr();
};
