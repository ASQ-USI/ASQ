...
module.exports = {
  ...
  injectQuestionsToHtml: coroutine(function * injectQuestionsToHtmlGen(html){
    ...
    const regex = new RegExp(`${asqServer}\\/questions\\/([0-9a-fA-F]{24})([A-Za-z0-9-._~:\\/?#\\[\\]@!$&'()*+,;=%]*)`, 'gi');
    ...
    if(qObjectIds.length === 0) return;

    const qHash = yield this.getQuestionsToInjectFromDb(qObjectIds);

    function replacer(match, id, slug, offset, string) {
      const html = `<asq-exercise>
    ${qHash[id].data.html}
    </asq-exercise>`
      return html
    }
