...
var listen = function(server) {
  ...
  io.of('/ctrl')
    ...
    .on('connection', function(socket) {
      ...
      socket.on('disconnect', function() {
        utils.ctrlDisconnect(io, socket);
      });
    });
  ...
  io.of('/folo')
    ...
    .on('connection', function(socket) {
      ...
      socket.on('asq:assess', function(evt){
        utils.foloAssess(io, socket, evt);
      });
       ...
       */
      socket.on('disconnect', function() {
        utils.foloDisconnect(io, socket);
      });
    });
  ...
  io.of('/wtap')
    ...
    .on('connection', function(socket) {
      /** handle connection **/
      utils.wtapConnect(io, socket);

      /*
       *  Handle the disconnection of a socket from the namespace.
       */
      socket.on('disconnect', function() {
        utils.wtapDisconnect(io, socket);
      });
    });
  ...
  io.of('/stat')
    ...
    .on('connection', function(socket) {
      /** handle connection **/
      utils.statConnect(io, socket);

      /*
       *  Handle the disconnection of a socket from the namespace.
       */
      socket.on('disconnect', function() {
        utils.statDisconnect(io, socket);
      });
    });
