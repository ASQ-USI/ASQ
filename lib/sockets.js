    ...
    ctrl.on('connection', function(socket) {
        ...
        socket.on('admin', function(event) {
            Session.findById(event.session, function(err, session) {
                ...
                socket.emit('goto', {slide:session.activeSlide});
            });
        });
            ...
            Session.findById(event.session, function(err, session) {
                session.question(function(err, question){
                    ...
                    //else send question
                        ...
                        ctrl.in(event.session).emit('question', {question: question});
                        folo.in(event.session).emit('question', {question: question});
    ...
    folo.on('connection', function(socket) {
        ...
        socket.on('viewer', function(event) {
            Session.findById(event.session, function(err, session) {
                ...
                socket.emit('goto', {slide:session.activeSlide});
                ctrl.in(event.session).emit('new', event);
            });
        });
