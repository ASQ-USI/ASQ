    ...
    folo.on('connection', function(socket) {
        ...
        socket.on('asq:submit', function(event) {
            socket.get('nickname', function(err, nickname) {
                if (err) throw err;
                Session.findById(getSession(socket.id), function(err, session) {
                   if(err) throw err;
                   Answer.findOne({_id: {$in: session.answers}, question: event.questionId},
                                  {upsert: true},
                                  function(err, answer){
                ...
                submission.answers = {user: user, content: event.answers};
