 ...
 //    }
  ...
  .authorization(function (handshakeData, callback) {
    ...
    if (!handshakeData.headers || !handshakeData.headers.cookie) {
      callback("Not authenticated or missing cookie");
    }

    console.log('parse cookie');
    var parsedCookie = cookie.parse(handshakeData.headers.cookie);
    var sessionID = connect.utils.parseSignedCookie(parsedCookie['asq.sid'], 'ASQSecret');
    console.log('Valid cookie: ' + !(parsedCookie['asq.sid'] == sessionID));
    console.log(parsedCookie['asq.sid']);
    console.log(sessionID);
    if (!(parsedCookie['asq.sid'] == sessionID)) {
      redisClient.get('sess:' + sessionID, function(err, expressSession) {
        if (err) { throw err; }
        var uid = JSON.parse(expressSession).passport.user;
        var sid = handshakeData.query.sid; // Needs to be checked for existence!
        ...
        //console.log(expressSession.cookie['passport']['user']);
      })
    }
    callback(null, true);
  });
