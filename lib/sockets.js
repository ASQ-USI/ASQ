...
module.exports.listen = function(server) {
      ...
      Session.findById(sessionId).exec()
        ...
        .then(
          function(session) {
            ...
            return currentSession.questionsForSlide(event.slide);
        })
        ...
        .then(
          ...
          currentSession.activeQuestions = questions;
 ...
 //    }
     ...
     */
            ...
            socket.get('nickname', function(err, nickname) {
                ...
                .then(
                  function(isQuestionInSlide){
                    ...
                    console.log (currentSession.answers)
                ...
                .then(
                  function(answer){
                    if (answer === null) {
                      ...
                      currentSession.save(function(err){throw err;})
		 ...
		 * namespace.
		 */
        socket.on('asq:submit2', function(event) {
          console.log(event.answers)
            var sessionId = getSession(socket.id);
            if (!sessionId || !event.questionId) return;
            socket.get('nickname', function(err, nickname) {
                ...
                Session.findById(sessionId, function(err, session) {
                    session.question(function(err, question) {
                        ...
                        Answer.findOne({_id: {$in: session.answers}, question:event.questionId}, function(err, answer) {
                            ...
                            //Notifies ctrl of participation
                            var submitted = 0;
                            for(var i = 0; i < answer.answers.length; i++) {
                                if (answer.answers[i].final) submitted++;
                            }
