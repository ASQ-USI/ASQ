...
module.exports  = {
  ...
  authorizePresenter: authentication.authorizePresenter,
  validateLdapUser   : ldap.validateLdapUser,


  /*
  * Will check if there's a Referer field in header and set the
  * session.redirect_to to the URL of the Referer field
  */
  setReferrerForRedirect: function(req, res, next){
    const redirect_to = req.header('Referer');
    if(redirect_to){
  ...
  forceSSL: function (req, res, next) {
    if (!req.secure) {
      ...
      return res.redirect(conf.rootUrl);
    }
    next(null);
  },
