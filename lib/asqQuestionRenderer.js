...
function renderStat($stat, question, userType){
  var deferred = when.defer();
  dustfs.render(defaultOptions.templates.stats[userType], question, function(err, out) {
    if(err) {
      deferred.reject(err)
    }
    $stat.html(out);
    $stat.attr('data-target-assessment-id', question.id);
    deferred.resolve(out);
  });
  return deferred.promise;
}
...
function renderAll($, questions, userType){
    ...
    _.each(questions, function(question){
      ...
      _.each($('.stats[data-target-assessment="' +question.htmlId+ '"]'), function(stat){
          deferreds.push(renderStat($(stat), question, userType));
      });
    })
