...
const createLiveSession = function (ownerId, presentation, presFlow, authLevel) {
...
const createImplicitAudienceQuestion = function (ownerId, sessionId) {
...
const createLivePresentationSession = coroutine(function *createLivePresentationSessionGen(ownerId, presentationId, presFlow, authLevel) {
  ...
  const newSession = this.createLiveSession(ownerId, presentation, presFlow, authLevel);

  /* Implicit question that receives audience questions as answers */
  const implicitQuestion = this.createImplicitAudienceQuestion(ownerId, newSession._id);

  /* Instantiate viewer question Exercise */
  const viewerQuestionExercise = this.createViewerQuestionExercise(implicitQuestion._id);
...
module.exports = {
  ...
  createLiveSession,
  createImplicitAudienceQuestion,
