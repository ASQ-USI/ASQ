...
const retrieveViewerQuestionById = coroutine(function*retrieveViewerQuestionByIdGen (questionId){
  const viewerQuestion = yield Answer
    ...
    .lean()
    .exec();
  viewerQuestion.id = viewerQuestion._id;
  delete viewerQuestion._id;
...
const handleSubmittedQuestion = coroutine(function*handleSubmittedQuestionGen (socket, evt) {
  ...
  const stem = 'viewerQuestionExercise';
...
const handleQuestionRated = coroutine(function*handleQuestionRatedGen (socket, evt){
  ...
  let namespaces = ['ctrl'];
  let evtName = 'student-question-rated';
  const data = {
  ...
  // notify clients 
  data.type = evtName;
  evtName = 'asq:live-app';
  namespaces = ['folo', 'ghost', 'ctrl'];
  notifyRoles(evtName, sessionId, data, namespaces);
...
/**
  Updates the session identified by sessionid with data regarding the active viewer question
  and broadcasts the event to the folo and ghost namespaces. 
  It is used for broadcasting a question (show it on the beamer and on the viewers screen), 
  or̨ hiding (go back to the normal presentation view).
  @param {String} evtName - The name of the event 
  @param {Object} data - The data that is being emitted
  @param {ObjectId} sessionId - The id of the current ongoing session
  @return {}
*/
const handleBroadcastStudentQuestionAndNotify = coroutine(function*handleBroadcastStudentQuestionAndNotifyGen (socket, evt) {
  const questionId = evt.questionId;
  const sessionId = socket.request.sessionId;

  // retrieve current session
  const session = yield retrieveSessionById(sessionId);
  ...
  const viewerQuestions = yield retrieveViewerQuestionsBySessionId(sessionId);
  ...
  const questionToForward = yield retrieveViewerQuestionById(questionId);
  ...
  evt.question = questionToForward;
  evt.header = 'User '+ questionToForward.submission.user +' asks:';
  evt.content = questionToForward.submission.value;
  const namespaces = ['folo', 'ghost', 'ctrl'];
  const evtName = 'asq:live-app';
  // LiveApp
  notifyRoles(evtName, sessionId, evt, namespaces);

  //Cockpit
  notifyRoles(evt.type, sessionId, evt, ['ctrl']);
});

const handleStudentQuestionsFunctionality = coroutine(function*handleStudentQuestionsFunctionalityGen(socket, evt) {
  // retrieve current session
  const sessionId = socket.request.sessionId;
  const session = yield retrieveSessionById(sessionId);
  if (!session) return new Error('Session with the given id could not be found');
  session.data = session.data || {};
  session.data.studentQuestionsEnabled = evt.studentQuestionsEnabled;
  yield Session
    .findById(sessionId)
    .update(session)
    .exec();
...
const handleCloseStudentQuestionBroadcastAndNotify = coroutine(function*handleCloseStudentQuestionBroadcastAndNotifyGen (socket, evt) {
  const questionId = evt.questionId;
  const sessionId = socket.request.sessionId;
...
module.exports = {
  ...
  handleQuestionRated,
  handleStudentQuestionsFunctionality,
  handleBroadcastStudentQuestionAndNotify,
  handleCloseStudentQuestionBroadcastAndNotify,
};
