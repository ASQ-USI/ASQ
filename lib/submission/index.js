/** @module lib/submission/index
    @description logic to handle submissions
*/
...
module.exports = {
  ...
  exerciseSubmission : coroutine(function *submitGen(socket, submission) {
    try{
      logger.debug('new submission:', require('util').inspect(submission));

      //get session and presentation
      var sessionId = socket.request.sessionId;
      var session = yield Session.findById(sessionId).populate('slides').exec();
      var presentation = session.slides;

      // make sure submission has valid formats
      ...
      submission.session = session._id;
     ...
     // this.afterExerciseSubmission(socket, submission.exerciseUid);
      ...
      var afterAnswers = yield Promise.map(processedAnswers, function(answer){
        ...
        return hooks.doHook("after_answer_submission", info);
      });
  ...
  validateSubmission: coroutine(function *validateSubmissionGen(submission){
    ...
    // console.log(questionUids)
    var exQ = exercise.questions.map(function(q){return q.toString()}).sort()
    var sQ = questionUids.map(function(q){return q.toString()}).sort()

    assert(_.isEqual(exQ, sQ),
  ...
  checkMaxSubmissionTimes: coroutine(function *checkMaxSubmissionTimes (submission) {
    ...
    var max = exercise.readSetting('maxNumSubmissions');

    // max <= -1 means infinite submissions
