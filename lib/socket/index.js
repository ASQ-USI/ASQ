...
const listen = function(server) {
  ...
  io.of('/ctrl')
    ...
    .on('connection', function(socket) {
      ...
      socket.on('share-student-question', function (questionId) {
        const evtName = 'share-student-question';
        const data = {
          questionId
        };
        const sessionId = this.request.sessionId;
        handlers.ctrlUpdateActiveViewerQuestionAndNotify(evtName, data, sessionId);
      });

      socket.on('toggle-student-questions', function (studentQuestionsEnabled) {
        const evtName = 'toggle-student-questions';
        const data = {
          studentQuestionsEnabled,
        };
        const sessionId = this.request.sessionId;
        handlers.ctrlUpdateStudentQuestionsEnabled(evtName, data, sessionId);
      });

      socket.on('close-modal', function () {
        const evtName = 'close-modal';
        const sessionId = this.request.sessionId;
        handlers.ctrlUpdateActiveViewerQuestionAndNotify(evtName, {}, sessionId);
      });
      /*
