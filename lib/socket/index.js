...
const listen = function(server) {
  ...
  io.of('/ctrl')
    ...
    .on('connection', function(socket) {
      ...
      socket.on('cockpit-message', presenterHandlers.handleRequest.bind(presenterHandlers, socket));
      ...
      socket.on('broadcast-student-question', function (questionId) {
        const type = 'broadcast-student-question';
        const data = {
          questionId,
          type,
        };
        liveApp.handleSocketEvent(socket, data);
      });

      socket.on('toggle-student-question-functionality', function (studentQuestionsEnabled) {
        const type = 'toggle-student-question-functionality'
        const data = {
      ...
      socket.on('close-question-broadcast', function () {
        const type = 'close-question-broadcast';
        const data = {
          type,
        }
        liveApp.handleSocketEvent(socket, data);
  ...
  io.of('/folo')
    ...
    .on('connection', function(socket) {
      ...
      /*
       *  Handle change coming from the live App.
       */
      socket.on('live-app', function(evt){
