...
const listen = function(server) {
  ...
  io.of('/ctrl')
    ...
    .on('connection', function(socket) {
      ...
      socket.on('share-student-question', function (question) {
        const evtName = 'share-student-question';
        handlers.ctrlForwardQuestionToRoles(evtName, question, question.session);
      });
      socket.on('update-student-questions', function (questions) {
        const evtName = 'update-student-questions';
        if (questions.length > 0) {
          const question = questions[0];
          handlers.ctrlForwardQuestionToRoles(evtName, questions, question.session);
        }
