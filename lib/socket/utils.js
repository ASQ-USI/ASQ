...
module.exports =  function(io, client, socketEmitter){
  ...
  var terminate = function terminate(socket, evt) {
    ...
    .then(
      ...
      function notifyTerminated(user) {
        emitToRoles('asq:session-terminated', {}, sid,
  ...
  socketEmitter.on('emitToRoles', function(evt){
   emitToRoles(evt.evtName, evt.event, evt.sessionId, evt.namespaces);
  });

  socketEmitter.on('emit', function(evt){
