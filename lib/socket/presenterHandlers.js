...
module.exports =  function(socketUtils){
  ...
  const hydrateCockpit = coroutine(function *hydrateCockpitGen(socket, evt){
    ...
    let sliderChangeEvents = yield SessionEvent
      .find({type: 'slider-changed', session: sessionId})
      .sort({time: 1})
      .lean()
      .exec();

    sliderChangeEvents = sliderChangeEvents.map(renameIdAndRemoveVersion);

    let ctrlGoToEventsTable = cockpitStatsHydration.generateCtrlGoToEventsTable(ctrlGoToEvents);
    
    const cockpitState = cockpitStatsHydration.generateRestoredCockpit(ctrlGoToEventsTable, sliderChangeEvents);
