...
module.exports =  function(socketUtils){
  ...
  const hydrateCockpit = coroutine(function *hydrateCockpitGen(socket, evt){
    ...
    //attach slide transitions to the session
    ...
    const currentSlide = studentPerceptionPerSlideTransition.pop();
    // This is for the current slide, so I restore the current lecture perception as it was
    const lecturePerception = currentSlide.perception.map(perception => {
      perception.data.id = perception.data.user.toString();
