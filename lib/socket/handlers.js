...
module.exports =  function(socketUtils){
  ...
  var updatePresentationSettings = coroutine(function *updatePresentationSettingsGen(socket, evt) {
    ...
    if ( evt.scope === 'presentation' ) {
      state = yield settings.updateSlideshowSettings(evt.settings, evt.presentationId);
      if ( state ) {
        exerciseSettings = yield settings.getDustifySettingsOfExercisesAll(evt.presentationId);
      }
    } else if ( evt.scope === 'exercise' ) {
      state = yield settings.updateExerciseSettings(evt.settings, evt.presentationId, evt.exerciseId);
    } 
    socket.emit('setting:update-presentation-settings-ack', {
      'state': state,
      'scope': evt.scope,
    ...
    if ( !userId ) return;
    var sessionId = yield presUtils.isLiveBy(userId, evt.presentationId);
    if ( !sessionId ) return;

    // This is a Live presentation.
    socketUtils.emitToRoles('setting:update-live-presentation-settings', evt, sessionId, 'ctrl');
    socketUtils.emitToRoles('setting:update-live-presentation-settings', evt, sessionId, 'folo');
  });
