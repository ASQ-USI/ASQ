...
module.exports =  function(socketUtils){
  ...
  var updatePresentationSettings = coroutine(function *updatePresentationSettingsGen(socket, evt) {
    ...
    if ( evt.scope === 'presentation' ) {
      state = yield settings.updateSlideshowSettings(evt.data, evt.presentationId);
      if ( state ) {
        data = yield settings.getDustifySettingsOfExercisesAll(evt.presentationId);
      }
    } else if ( evt.scope === 'exercise' ) {
      state = yield settings.updateExerciseSettings(evt.data, evt.presentationId, evt.exerciseId);
    } 
    ...
    socketUtils.emitToRoles('setting:update-presentation-settings-live', evt, sessionId, 'ctrl');
    socketUtils.emitToRoles('setting:update-presentation-settings-live', evt, sessionId, 'folo');
