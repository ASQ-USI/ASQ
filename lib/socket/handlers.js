...
module.exports =  function(socketUtils){
  ...
  const ctrlConnect = coroutine(function *ctrlConnectGen(socket) {
    ...
    try{
      ...
      socketUtils.sendConnectedClients(sessionId.toString(), 'ctrl');
  ...
  const ctrlDisconnect = function(socket) {
    ...
    .then(
     ...
     .then(
      function delAndNotify(userInfo) {
        return socketUtils.sendConnectedClients(sid.toString(), 'ctrl');
  ...
  const foloDisconnect = function(socket) {
    ...
    coroutine(function *notifyProgressGen(data) {
      ...
      socketUtils.sendConnectedClients(sid.toString(), 'ctrl');
  ...
  const ctrlForwardDataToRoles = coroutine( function *ctrlForwardDataToRolesGen (evtName, data, sessionId) {
    ...
    const type = 'student-question-submitted';
    const studentQuestionsData = {
      type,
      session: sessionId,
    };
    // Retrieving all the student questions that were made during the session
    const viewerQuestions = yield Answer
    ...
    yield Session
      ...
      .exec();
    if (questionId === null) {
      socketUtils.emitToRoles(evtName, null, sessionId, 'folo');
      socketUtils.emitToRoles(evtName, null, sessionId, 'ghost');
      return;
    }
    // Once the session has been updated, retrieve the question to forward and forward it
    const questionToForward = yield Answer
      .findById(questionId)
      .lean()
      .exec();
    // removing private variables
    questionToForward.id = questionToForward._id;
    delete questionToForward._id;
    socketUtils.emitToRoles(evtName, questionToForward, sessionId, 'folo');
    socketUtils.emitToRoles(evtName, questionToForward, sessionId, 'ghost');
  });
  ...
  return {
    ...
    ctrlForwardDataToRoles: ctrlForwardDataToRoles
  }
}
