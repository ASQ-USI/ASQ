...
module.exports =  function(socketUtils){
  ...
  const ctrlForwardDataToRoles = coroutine( function *ctrlForwardDataToRolesGen (evtName, data, sessionId) {
    ...
    if (question !== undefined) dataToForward = question;
    else if (questions !== undefined) dataToForward = questions;
    ...
    let questionId = session.data.activeViewerQuestion;
    if (question)
      questionId = question.id;

    let viewerQuestions = session.data.viewerQuestions;
    ...
    yield Session
      ...
      .update(session)
      .exec();
    socketUtils.emitToRoles(evtName, dataToForward, sessionId, 'folo');
    socketUtils.emitToRoles(evtName, dataToForward, sessionId, 'ghost');
  });
