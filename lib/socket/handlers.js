...
module.exports =  function(socketUtils){
  ...
  var updatePresentationSettings = coroutine(function *updatePresentationSettingsGen(socket, evt) {
    console.log('updatePresentationSettings');
    ...
    if ( !userId ) {
      socket.emit('asq:update_presentation_settings_ack', {'state': false});
      return
    }
    ...
    var sessionId = yield presUtils.getSessionIfLiveBy(userId, evt.presentationId);
    if ( !sessionId ) return;
