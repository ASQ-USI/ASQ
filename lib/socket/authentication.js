...
module.exports = {
   ...
   */
  createSocketToken: function createSocketToken(profile){
  ...
  ctrlAuthorize: function ctrlAuthorize(socket, next) {  
    ...
    var user = socket.user = socket.decoded_token.user;
  ...
  ownerAuthorize: function onwerAuthorize(socket, next) { 
    var request = socket.request;
    // Check if the session was sent.
    if (!request._query || !request._query.asq_sid) {
      process.nextTick( function(){
    ...
    // TODO: I want to check if the user is the onwer of the presentation
    // but I don't know how to get the presentation id here
    next();
  },
