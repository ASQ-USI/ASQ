...
const presentationsApi = require('../../api/presentations.js');
...
resources.questions = coroutine(function *handleQuestionsGen(socket, evt){
  switch (evt.body.method){
    case 'read':
      ...
      socket.send(module.exports.createResponseEvent( evt, questions));
      break;
    default:
...
resources.presentations = coroutine(function *handlePresentationsGen(socket, evt){
  switch (evt.body.method){
    case 'read':
      const presentations = yield presentationsApi.list();
      socket.send(module.exports.createResponseEvent(socket, evt, presentations));
      break;
    default:
      console.log('TODO: handle `socket/api/index:resources.presentations()` error');
  }
});

module.exports = {
  createResponseEvent: function(reqEvt, payload){
    return {
      type: 'response',
      reply_to: reqEvt.id,
